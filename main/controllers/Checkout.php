	...
	public function index() {
		...
		if ( ! $this->customer->islogged() AND $this->config->item('guest_order') !== '1') { 											// else if customer is not logged in
			$this->alert->set('alert', $this->lang->line('alert_customer_not_logged'));
  			redirect('login'.$prepend);															// redirect to account register page and display error
		}
        ...
        $data['text_login_register']	= $this->customer->isLogged() ? sprintf($this->lang->line('text_logout'), $this->customer->getFirstName(), site_url('account/logout'.$prepend)) : sprintf($this->lang->line('text_registered'), site_url('login'.$prepend));
